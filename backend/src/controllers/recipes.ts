...
export const getRecipe = async (req: AuthRequest, res: Response) => {
  ...
  const [{ data: recipe, error }, { data: ingredients }, { data: steps }] =
    await Promise.all([
      supabase.from('recipes').select('*, profiles(username)').eq('id', id).single(),
...
export const createRecipe = async (req: AuthRequest, res: Response) => {
  const { title, description, prep_time, cook_time, servings, image_url, ingredients, steps } = req.body;

const { data: recipe, error } = await supabase
  .from('recipes')
  .insert({ title, description, prep_time, cook_time, servings, image_url, user_id: req.user!.id })
  .select()
  .single();  
...
export const updateRecipe = async (req: AuthRequest, res: Response) => {
  ...
  const { data, error } = await supabase
  .from('recipes')
  .update({ title, description, prep_time, cook_time, servings, image_url, updated_at: new Date().toISOString() })
  .eq('id', id)
